...
def scrape(path):
    ...
    for menu in menus:
        ...
        for pref in prefs:
            # Add breakfast data
            if re.search(rf'>[^<]*{pref['term']}[^>]*<', page_source_dict_lst[0]['src'], re.IGNORECASE):
                pref['breakfast'] += menu
            # Add brunch/lunch data
            if re.search(rf'>[^<]*{pref['term']}[^>]*<', page_source_dict_lst[1]['src'], re.IGNORECASE):
                pref['brunch_lunch'] += menu
            # Add dinner data
            if re.search(rf'>[^<]*{pref['term']}[^>]*<', page_source_dict_lst[2]['src'], re.IGNORECASE):
                pref['dinner'] += menu
